...
public class VRMouseLook : MonoBehaviour {
...
#else
    const bool mouseLookEnabled = true;
	...
	void Update () {
		...
		if (Input.GetKey(HorizontalAndVerticalKey)) {
			pitched = true;
			if (enableYaw) {
				mouseX += Input.GetAxis("Mouse X") * 5 * sensitivity;
				if (mouseX <= -180) {
					mouseX += 360;
				} else if (mouseX > 180) {
					mouseX -= 360;
			...
			mouseY -= Input.GetAxis("Mouse Y") * 2.4f * sensitivity;
			mouseY = Mathf.Clamp(mouseY, -85, 85);
		} else if (Input.GetKey(RollKey)) {
			rolled = true;
			mouseZ += Input.GetAxis("Mouse X") * 5 * sensitivity;
